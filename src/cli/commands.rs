...
/// Map a user key string to a SpecialKey serde name (the variant name verbatim,
/// e.g. "Enter", "CtrlC"). Supports the canonical names plus friendly aliases.
fn map_special_key(key: &str) -> Result<&'static str, String> {
    ...
    let mapped = match k.as_str() {
        ...
        _ => {
            return Err(format!(
                "Unknown key: {key}\nValid keys: enter, esc, ctrl-c, ctrl-d, ctrl-z, tab, up, down, left, right, home, end, pageup, pagedown, backspace, delete"
            ));
        }
    };
    Ok(mapped)
...
pub fn cli_ls(json_mode: bool) -> i32 {
    ...
    if json_mode {
        return match api_get("/v1/state", &token) {
            Ok(body) => {
                println!("{body}");
                0
            }
...
pub fn cli_project_add(
    ...
    if hidden {
        ...
        if let Some(w) = window {
            apply_window(&mut hide_body, &token, w);
        }
...
/// Prints to stderr and returns Err on failure; the caller decides what to do.
fn apply_window(body: &mut serde_json::Value, token: &str, window: &str) {
    match fetch_state(token).and_then(|s| resolve::resolve_window(&s, window)) {
        Ok(win) => {
...
/// `okena project rm <project> [--force]`
pub fn cli_project_rm(project: &str, _force: bool) -> i32 {
...
pub fn cli_term_split(terminal: &str, direction: &str) -> i32 {
    ...
    let p = state
        ...
        .iter()
        .find(|p| p.id == project_id)
        .expect("project resolved from state");
