                    ...
                    toolbar_button("ov-pause", icon, label, &t)
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(l) = this.active_loops.get_mut(focused_idx) {
                                l.paused = !l.paused;
                                l.state = if l.paused { LoopState::Paused } else { LoopState::Running };
                            }
                            cx.notify();
                        })),
                )
            ...
            .when(!is_completed && focused.is_some(), |el| {
                el.child(
                    toolbar_button("ov-skip", "icons/skip-forward.svg", "Skip", &t)
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(l) = this.active_loops.get_mut(focused_idx) { l.skip_requested = true; }
                            cx.notify();
                        })),
                )
            })
            .when(!is_completed && focused.is_some(), |el| {
                el.child(
                    toolbar_button_toggle("ov-step", "icons/step-forward.svg", "Step", is_step, &t)
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(l) = this.active_loops.get_mut(focused_idx) { l.step_mode = !l.step_mode; }
                            cx.notify();
                        })),
                )
            })
            .when(is_waiting, |el| {
                el.child(
                    toolbar_button_primary("ov-continue", "icons/play.svg", "Continue", &t)
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(l) = this.active_loops.get_mut(focused_idx) { l.step_advance_requested = true; }
                            cx.notify();
                        })),
                )
            })
            ...
            .when(multi_loop && focused.is_some() && !is_completed, |el| {
                el.child(
                    toolbar_button("ov-quit-one", "icons/close.svg", "Quit", &t)
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if let Some(l) = this.active_loops.get_mut(focused_idx) { l.quit_requested = true; }
                            cx.notify();
                        })),
                )
            })
            .child({
                let (icon, label) = if all_done { ("icons/chevron-left.svg", "Back") }
                    else if multi_loop { ("icons/close.svg", "Quit All") }
                    else { ("icons/close.svg", "Quit") };
                toolbar_button("ov-quit-all", icon, label, &t)
                    .on_click(cx.listener(|this, _, _, cx| { this.close_loops(cx); }))
            })
            .child(div().flex_1().min_w(SPACE_SM))
            .child(
                div().flex().gap(SPACE_MD).items_center()
                    .child(div().text_size(TEXT_SM).font_weight(FontWeight::SEMIBOLD).text_color(rgb(state_color)).child(state_label)),
            )
    }
