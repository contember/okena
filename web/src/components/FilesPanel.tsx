...
export function FilesPanel({
  ...
  const loadDirectory = useCallback(async () => {
    ...
    try {
      ...
      setDirectoryState({
        status: "idle",
        entries: parseDirectoryEntries(payload),
      });
    } catch (error) {
      setDirectoryState({
        status: "error",
  ...
  return (
    <aside
      className={`flex h-full flex-shrink-0 flex-col bg-zinc-950 ${
        fullWidth ? "w-full" : "w-80 border-l border-zinc-800"
      }`}
    >
      ...
      <div className="h-44 flex-shrink-0 overflow-auto border-b border-zinc-800 px-2 py-1">
        {directoryState.status === "error" ? (
          <div className="px-1 py-1 text-xs text-red-400">
