...
impl GitHeader {
    pub(super) fn show_diff_popover(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this: WeakEntity<Self>, cx| {
            ...
            let result = smol::unblock(move || provider.get_diff_file_summary()).await;
            ...
            let _ = this.update(cx, |this, cx| {
                ...
                match result {
                    Ok(summaries) if summaries.is_empty() => return,
                    Ok(summaries) => {
                        this.diff_file_summaries = summaries;
                        this.diff_popover_error = None;
                    }
                    Err(error) => {
                        this.diff_file_summaries.clear();
                        this.diff_popover_error = Some(error);
                    }
                }
    ...
    pub fn render_diff_popover(&self, t: &ThemeColors, cx: &mut Context<Self>) -> AnyElement {
        if !self.diff_popover_visible
            || (self.diff_file_summaries.is_empty() && self.diff_popover_error.is_none())
        {
        ...
        let body = if let Some(error) = &self.diff_popover_error {
            div()
                .px(px(12.0))
                .py(px(10.0))
            ...
            div()
                .children(project_header::render_diff_file_list_interactive(
