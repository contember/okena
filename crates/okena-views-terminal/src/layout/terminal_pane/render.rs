...
impl<D: ActionDispatch + Send + Sync> Render for TerminalPane<D> {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .on_action(cx.listener(|this, _: &JumpToPreviousPrompt, _window, cx| {
                this.handle_jump_prev_prompt(cx);
            }))
            .on_action(cx.listener(|this, _: &JumpToNextPrompt, _window, cx| {
                this.handle_jump_next_prompt(cx);
            }))
            .on_action(
                cx.listener(|this, _: &JumpToPreviousFailedCommand, _window, cx| {
                    this.handle_jump_prev_failed(cx);
                }),
            )
            .on_action(
                cx.listener(|this, _: &JumpToNextFailedCommand, _window, cx| {
                    this.handle_jump_next_failed(cx);
                }),
            )
