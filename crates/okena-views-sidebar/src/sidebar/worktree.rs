...
impl Sidebar {
    /// Resolve the remote action client for a project. Returns `None` if the
    ...
    fn resolve_remote_params(
        ...
        let client = okena_transport::remote_action::RemoteActionClient::new(config, token);
    ...
    pub fn spawn_quick_create_worktree(&mut self, project_id: &str, cx: &mut Context<Self>) {
        ...
        let Some((client, daemon_id)) = self.resolve_remote_params(project_id, cx) else {
