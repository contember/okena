...
use crate::input_overlay::{InputOverlay, PredictedCell};
...
impl Terminal {
    ...
    pub fn process_output(&self, data: &[u8]) {
        ...
        *self.last_output_time.lock() = Instant::now();
        ...
        // GC expired predictions on output
        if self.is_remote {
            drop(processor);
            drop(term);
    ...
    pub fn predict_char(&self, c: char) -> Option<u64> {
        ...
        log::trace!("[prediction] char='{}' seq={} cursor=({},{})", c, seq, cursor.column.0, cursor.line.0);
        Some(seq)
    }
    ...
    pub fn ack_predictions(&self, seq: u64) {
        ...
        let mut overlay = self.input_overlay.lock();
        let pending_before = overlay.cells().len();
        overlay.on_server_frame(seq, (cursor.column.0, cursor.line.0));
        let pending_after = overlay.cells().len();
        if pending_before != pending_after {
            log::debug!("[prediction] ack seq={} cleared {} predictions ({} remaining)",
                seq, pending_before - pending_after, pending_after);
        }
    }
